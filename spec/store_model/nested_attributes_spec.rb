...
RSpec.describe StoreModel::NestedAttributes do
  ...
  describe ".reflect_on_association" do
    context "when active_admin_compatibility is enabled" do
      before do
      ...
      context "when association is a StoreModel array type" do
        ...
        it "returns a Reflection object" do
          expect(subject).to be_a(StoreModel::ActiveAdminCompatibility::Reflection)
        end
      ...
      context "when accessing via symbol or string" do
        it "works with symbol" do
          expect(model_class.reflect_on_association(:suppliers)).to be_a(StoreModel::ActiveAdminCompatibility::Reflection)
        end
    ...
    context "when active_admin_compatibility is disabled" do
      let(:model_class) do
        Class.new(ActiveRecord::Base) do
          include StoreModel::NestedAttributes
      ...
      it "does not respond to reflect_on_association for StoreModel attributes" do
        expect(model_class.reflect_on_association(:suppliers)).to be_nil
      end
    end
  end
  ...
  describe ".store_model_reflections" do
    context "when active_admin_compatibility is enabled" do
      ...
      it "returns hash of reflections" do
        ...
        expect(subject[:suppliers]).to be_a(StoreModel::ActiveAdminCompatibility::Reflection)
      end
    end
  end
end
